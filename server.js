...
setInterval(() => {
  http.get(`http://systemboda.glitch.me/`);
}, 280000);
...
client.on('message', message => {
    if (message.content.startsWith("كسملي نوفل")) {
      message.channel.send("كسمه")
message.author.send('كسمه لاجلك يعمهم')
    }
});
...
client.on('message', msg => {
 if (msg.content.startsWith(prefix + 'dm')) {
   if (!msg.member.hasPermission("ADMINISTRATOR")) return msg.channel.send("Error");
      let args = msg.content.split(' ').slice(2)
      let alpha = msg.mentions.members.first()
      if (!alpha) return msg.reply(`**يجب تحديد الشخص**`)

   
      client.users.get(`${alpha.id}`).send(args.join(" "))
     ...
     var codes = args.join(' ')
        ...
        if(!codes) {
          message.channel.send("قم بكتابة الرسالة | !rolebc @everyone message")
            ...
            message.guild.members.filter(m => m.members.get(role.id)).forEach(n => {
              n.send(
              `${codes}` + ""
              )
            })
            message.channel.send(`لقد تم ارسال هذه الرسالة الى ${message.guild.members.filter(m => m.members.get(role.id)).size} عضو`)
        }
    });
...
client.login('NzMzMzkxNzMwMTM2NzExMjcw.XxCeRg.oFbP68oNxfvOiToxKt8ZBvmkQSY');
